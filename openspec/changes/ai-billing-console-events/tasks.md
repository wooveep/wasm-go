...
- [ ] 3.5 Map parsed token counts to `usage.unit`, `usage.input`, `usage.output`, `usage.total`, and `usage.details`, using `input + output` when total is missing.
...
- [ ] 5.1 Update config parsing tests for `billing_service.auth_token` and default `consumer_header`.
