...
- [ ] 1.1 Run GitNexus upstream impact analysis for `recordUsage`, `onHttpResponseBody`, `onHttpStreamingResponseBody`, and `GetTokenUsage`; report direct callers, affected processes, and risk before editing those symbols.
- [ ] 1.2 Inspect the existing `ai-billing` request, non-streaming response, streaming response, and billing delivery flows to identify current context keys and event construction points.
