...
- **Responses Protocol**: Request path `/v1/responses`, preferring native provider Responses support and falling back to Chat Completions when needed
...
> When the request path suffix matches `/v1/responses`, it corresponds to OpenAI Responses API scenarios. The plugin automatically detects provider capabilities: if `capabilities` includes `openai/v1/responses`, requests are forwarded natively; otherwise, when the provider supports `openai/v1/chatcompletions`, requests are converted to Chat Completions and non-streaming responses or streaming SSE are converted back to Responses format.
...
| `capabilities`   | map of string          | Optional    | -       | Enables native forwarding for provider APIs that are already compatible with an OpenAI-style capability. Keys are protocol capabilities and values are upstream API paths. Supported capability keys include `openai/v1/chatcompletions`, `openai/v1/responses`, `openai/v1/embeddings`, `openai/v1/imagegeneration`, `openai/v1/audiospeech`, and `cohere/v1/rerank`. |
...
Only non-streaming requests are currently retried.

| Name            | Data Type       | Requirement | Default        | Description                                                                                          |
| --------------- | --------------- | ----------- | -------------- | ---------------------------------------------------------------------------------------------------- |
| `enabled`       | bool            | Optional    | false          | Whether to enable failed request retry.                                                              |
| `maxRetries`    | int             | Optional    | 1              | Maximum number of retries.                                                                           |
| `retryTimeout`  | int             | Optional    | 60000          | Retry request timeout in milliseconds.                                                               |
| `retryOnStatus` | array of string | Optional    | ["4.*", "5.*"] | Original request status code patterns that trigger retry. Regular expressions are supported.          |
...
| `qwenEnableCompatible` | boolean          | Optional | true          | Enable Qwen compatibility mode. When Qwen compatibility mode is enabled, the compatible mode interface of Qwen will be called, and the request/response will not be modified. |
...
The plugin now supports automatic protocol detection, capable of handling OpenAI, Claude, and Responses protocol format requests simultaneously.
...
**Responses Protocol Request Example**

URL: `http://your-domain/v1/responses`
...
If the provider supports the Responses API natively, declare the native path in `capabilities`; the plugin will forward `/v1/responses` directly:
...
If `openai/v1/responses` is not declared but the provider supports `openai/v1/chatcompletions`, the plugin converts the supported Responses request subset to Chat Completions and converts non-streaming responses or streaming SSE back to Responses format. Fallback mode supports text `input`, message-array input made of `input_text`/`output_text`, `instructions`, common generation parameters, function tools, and compatible `tool_choice` values. Fallback mode does not support `previous_response_id`, `conversation`, hosted tools, file inputs, or multimodal inputs such as images.

**Example Response**

Each protocol format returns responses in its respective format:
