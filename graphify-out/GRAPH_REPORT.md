# Graph Report - wasm-go  (2026-05-21)

## Corpus Check
- 127 files · ~103,323 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2800 nodes · 3398 edges · 199 communities (170 shown, 29 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 251 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8397a589`
...
1. `redisCallInternal()` - 62 edges
2. `respString()` - 61 edges
3. `RedisClusterClient[C]` - 61 edges
4. `NewTestHost()` - 45 edges
5. `CommonHttpCtx[PluginConfig]` - 41 edges
6. `RunTest()` - 40 edges
7. `mockBillingHttpContext` - 35 edges
8. `提供商特有配置` - 32 edges
9. `用法示例` - 29 edges
10. `TestHost` - 28 edges
...
Cohesion: 0.05
Nodes (85): TestBillingEventDelivery(), quotaConfigWith(), TestMissingPriceScriptResult(), TestMonetaryDeduction(), TestParseConfig(), TestRequestAdmission(), TestBuiltinAttributesDefaultSource(), TestCompleteFlow() (+77 more)
...
Cohesion: 0.07
Nodes (57): AIStatisticsConfig, Attribute, convertToUInt(), debugLogAiLog(), extractClaudeStreamingToolCalls(), extractSessionId(), extractStreamingBodyByJsonPath(), extractStreamingToolCalls() (+49 more)
...
Cohesion: 0.08
Nodes (28): Log, Critical(), Criticalf(), Debug(), Debugf(), Error(), Errorf(), Info() (+20 more)
...
Cohesion: 0.09
Nodes (23): ADDED Requirements, ai-plugin-resource-docs Specification, Purpose, Requirement: ai-billing resource package is generated, Requirement: ai-billing resource schema matches plugin configuration, Requirement: ai-quota resource documentation reflects monetary quota behavior, Requirement: ai-quota resource schema matches monetary quota configuration, Requirement: Resource metadata stays aligned with extension examples (+15 more)
...
Cohesion: 0.14
Nodes (20): BillingConfig, BillingEvent, BillingService, BillingUsage, billingUsageDetails(), buildBillingEvent(), deliverBillingEvent(), int64FromContext() (+12 more)
...
Cohesion: 0.11
Nodes (18): ADDED Requirements, ai-billing-events Specification, Purpose, Requirement: Billing event delivery is fail-open by default, Requirement: Billing events are generated per completed AI request, Requirement: Billing plugin configuration, Requirement: Billing plugin does not mutate account balance, Requirement: Billing plugin is independent from quota and statistics plugins (+10 more)
...
Cohesion: 0.15
Nodes (14): initBillingRequestContext(), isBillingDeliveryAcceptedStatus(), isBillingDeliveryFailureStatus(), newEventID(), sendBillingEvent(), statusCodeFromHeaders(), TestBillingDeliveryAcceptedStatusExcludesAuthFailures(), TestBillingDeliveryFailureStatusClassification() (+6 more)
...
Cohesion: 0.17
Nodes (12): code:yaml (provider:), code:yaml (provider:), code:yaml (provider:), code:json ({), code:yaml (provider:), code:json ({), code:json ({), 使用 OpenAI 协议代理 MiniMax 服务 (+4 more)
...
Cohesion: 0.31
Nodes (7): code:yaml (redis:), code:text (ceil(input_tokens * input_price / price_unit_tokens)), Configuration, Example, Overview, Redis Keys, Runtime Properties

### Community 94 - "Community 94"
Cohesion: 0.28
Nodes (7): code:yaml (billing_service:), Event 字段, 事件字段, 功能说明, 运行属性, 配置示例, 配置说明
...
Cohesion: 0.46
Nodes (8): init(), init(), init(), ProcessRequestHeaders(), ProcessResponseBody(), ProcessResponseHeaders(), ProcessStreamingResponseBody(), SetCtx()
...
## Knowledge Gaps
- **1301 isolated node(s):** `Log`, `PluginContext`, `HttpContext`, `oldParseConfigFunc`, `oldParseRuleConfigFunc` (+1296 more)
...
- **Why does `recoverFunc()` connect `Community 9` to `Community 0`, `Community 3`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Why does `TestBillingEventDelivery()` connect `Community 0` to `Community 47`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `UnsafeInfof()` connect `Community 5` to `Community 9`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Are the 42 inferred relationships involving `NewTestHost()` (e.g. with `TestParseConfig()` and `TestOnHttpRequestHeaders()`) actually correct?**
  _`NewTestHost()` has 42 INFERRED edges - model-reasoned connections that need verification._
- **What connects `Log`, `PluginContext`, `HttpContext` to the rest of the system?**
