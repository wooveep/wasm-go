# Graph Report - wasm-go  (2026-06-03)

## Corpus Check
- 153 files · ~129,863 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3251 nodes · 3969 edges · 255 communities (225 shown, 30 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 304 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3cf05a61`
...
2. `respString()` - 61 edges
3. `RedisClusterClient[C]` - 61 edges
4. `NewTestHost()` - 52 edges
5. `RunTest()` - 43 edges
6. `CommonHttpCtx[PluginConfig]` - 41 edges
7. `mockBillingHttpContext` - 35 edges
8. `contains()` - 34 edges
9. `提供商特有配置` - 32 edges
10. `用法示例` - 29 edges
...
## Communities (255 total, 30 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.06
Nodes (69): AIStatisticsConfig, Attribute, convertToUInt(), debugLogAiLog(), extractClaudeStreamingToolCalls(), extractSessionId(), extractStreamingBodyByJsonPath(), extractStreamingToolCalls() (+61 more)
...
Cohesion: 0.05
Nodes (36): Log, Critical(), Criticalf(), Debug(), Debugf(), Error(), Errorf(), Info() (+28 more)
...
Cohesion: 0.05
Nodes (39): Built-in Attributes, code:yaml ('{"ai_log":"%FILTER_STATE(wasm.ai_log:PLAIN)%"}'), code:json ({), code:bash (# Log format example), code:yaml (attributes:), code:block13 (route_upstream_model_consumer_metric_input_token{ai_route="b), code:json ({), code:yaml (attributes:) (+31 more)
...
Cohesion: 0.12
Nodes (25): BillingConfig, BillingEvent, BillingFact, BillingService, BillingUsage, billingUsageDetails(), buildBillingEvent(), deliverBillingEvent() (+17 more)
...
Cohesion: 0.17
Nodes (23): assertNoRedisMutationQueries(), quotaConfigWith(), TestAdmissionOnlyDoesNotDeduct(), TestKeyBuilders(), TestMissingPriceScriptResult(), TestMonetaryDeduction(), TestParseConfig(), TestRequestAdmission() (+15 more)
...
Cohesion: 0.16
Nodes (7): ClusterClient, ClusterClient[C], HttpCall(), redactSensitiveRequestHeaders(), TestRedactSensitiveRequestHeaders(), HttpClient, ResponseCallback
...
Cohesion: 0.16
Nodes (11): TestBusyLoopLinearity(), TestComplexHttpCall(), TestComplexHttpCallWithDifferentLoops(), TestHttpCall(), headerOption, HeaderOptionFunc, NewTestHostWithForeignFuncs(), WithEndOfStream() (+3 more)
...
Cohesion: 0.2
Nodes (13): isBillingDeliveryAcceptedStatus(), isBillingDeliveryFailureStatus(), sendBillingEvent(), statusCodeFromHeaders(), deliverTestBillingEvent(), mustBillingConfig(), requireObjectFactName(), TestBillingDeliveryAcceptedStatusExcludesAuthFailures() (+5 more)
...
Cohesion: 0.39
Nodes (7): init(), main(), onHttpRequestBody(), onHttpRequestHeaders(), parseConfig(), RequestBlockConfig, ProcessRequestBody()
...
Cohesion: 0.22
Nodes (7): ADDED Requirements, Requirement: Credential uniqueness, Requirement: Realm response header, Scenario: Configured realm is used, Scenario: Default realm is used, Scenario: Duplicate credential across consumers is rejected, Scenario: Duplicate credential in same consumer is rejected
...
Cohesion: 0.57
Nodes (7): parseBillingService(), parseConfig(), parseConfigFields(), parsePathSuffixes(), parseRuleConfig(), stringDefault(), NewClusterClient()
...
Cohesion: 0.29
Nodes (7): code:block10 (route_upstream_model_consumer_metric_input_token{ai_route="b), code:json ({), code:yaml (attributes:), 从非 openai 协议提取 token 使用信息, 日志, 监控, 链路追踪
...
Cohesion: 0.29
Nodes (6): key-auth-local-yaml Specification, Purpose, Requirement: Consumer single credential compatibility, Requirements, Scenario: Existing single credential config authenticates, Scenario: Existing single credential config injects consumer
...
Cohesion: 0.5
Nodes (4): code:block4 (# counter, cumulative count of input tokens), code:block5 (irate(route_upstream_model_consumer_metric_llm_first_token_d), code:block6 (irate(route_upstream_model_consumer_metric_llm_service_durat), Metric
...
## Knowledge Gaps
- **1576 isolated node(s):** `RuleConfig[PluginConfig]`, `ResponseCallback`, `HttpClient`, `ClusterClient`, `Log` (+1571 more)
...
- **Why does `contains()` connect `Community 0` to `Community 1`, `Community 36`, `Community 9`, `Community 108`, `Community 47`, `Community 50`, `Community 86`, `Community 23`, `Community 22`, `Community 60`, `Community 29`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
- **Why does `redisCallInternal()` connect `Community 1` to `Community 0`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Why does `TestBillingEventDelivery()` connect `Community 60` to `Community 0`, `Community 33`, `Community 36`, `Community 47`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Are the 49 inferred relationships involving `NewTestHost()` (e.g. with `TestHttpCall()` and `TestRebuildExample()`) actually correct?**
  _`NewTestHost()` has 49 INFERRED edges - model-reasoned connections that need verification._
- **Are the 39 inferred relationships involving `RunTest()` (e.g. with `TestComplexHttpCall()` and `TestComplexHttpCallWithDifferentLoops()`) actually correct?**
  _`RunTest()` has 39 INFERRED edges - model-reasoned connections that need verification._
