...
- **Are the 48 inferred relationships involving `NewTestHost()` (e.g. with `TestHttpCall()` and `TestRebuildExample()`) actually correct?**
  _`NewTestHost()` has 48 INFERRED edges - model-reasoned connections that need verification._
